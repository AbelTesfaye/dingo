...
package com.dingo.floatingview;
...
import com.dingo.PIPVideoPlayer.PIPVideoPlayerService;
...
class FloatingView extends FrameLayout implements ViewTreeObserver.OnPreDrawListener {
    ...
    private void refreshLimitRect() {
        ...
        newScreenWidth = mMetrics.widthPixels;
        newScreenHeight = mMetrics.heightPixels;
        ...
        mPositionLimitRect.set(-mOverMargin, -mOverMargin, newScreenWidth - width + mOverMargin + mNavigationBarHorizontalOffset, newScreenHeight + mOverMargin - mStatusBarHeight - height + mNavigationBarVerticalOffset);
    ...
    public boolean dispatchTouchEvent(@NonNull MotionEvent event) {
        ...
        // 押上、キャンセル
            ...
            if (this.getAlpha() != 1.0f) {
                animators.add(ObjectAnimator.ofFloat(this, "alpha", 1.0f));
            }

            if (animators != null) {
                if(slideOut)
                    animators.add(ObjectAnimator.ofFloat(this, "alpha", 0.0f));
                ...
                final boolean closeService = slideOut;
                animatorSet.addListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        if(closeService) {
                            Intent myService = new Intent(getContext(), PIPVideoPlayerService.class);
        ...
        if (isWaitForMoveToEdge && mAnimationHandler.getState() != STATE_FINISHING) {
            ...
            if (slideOut) {
                ...
                final int goalPositionY = getGoalPositionY(currentX, currentY);

                if (slideLeft) {
                    moveTo(currentX, currentY, -getWidth(), goalPositionY, true, false);
                } else {
                    moveTo(currentX, currentY, newScreenWidth, goalPositionY, true, false);
                }
            }else {
                moveToEdge(true);
            }
    ...
    private void onLongClick() {
        ...
        int newWidth = mInitialViewWidth;
        if(isExpanded){
            isExpanded = false;
        }
        else {
            newWidth = mPositionLimitRect.width() + getWidth();
            isExpanded = true;
        }
        ...
        int touchY = (int) (mMetrics.heightPixels + mNavigationBarVerticalOffset
                - (mScreenTouchY - mLocalTouchY + newHeight - mTouchYOffset));

        mAnimationHandler.updateTouchPosition(touchX, touchY);
     ...
     */
        private void moveTo(int currentX, int currentY, int goalPositionX, int goalPositionY, boolean withAnimation) {
            moveTo(currentX, currentY, goalPositionX, goalPositionY, withAnimation, true);
        }
        private void moveTo(int currentX, int currentY, int goalPositionX, int goalPositionY, boolean withAnimation, boolean clampToLimitRect) {
        // 画面端からはみ出さないように調整
        if(clampToLimitRect) {
        ...
        // タッチ座標を初期化
            if(mInitialViewWidth == this.getWidth() && !mIsLongPressed){
    ...
    public class OnPinchListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        // The default constructor pass context and frameLayout object.
        public OnPinchListener(Context context, FrameLayout frameLayout) {
        ...
        private void scale(float scale)
        {
            ...
            int maxWidthWithinBoundary = mPositionLimitRect.width() + getWidth();
            int minWidthAllowed = mInitialViewWidth/2;
            if(newWidth == maxWidthWithinBoundary || newWidth == minWidthAllowed)
                return;

            else if(newWidth > maxWidthWithinBoundary){
                newWidth = maxWidthWithinBoundary;
            }
            else if(newWidth < minWidthAllowed){
                newWidth = minWidthAllowed;
            }
