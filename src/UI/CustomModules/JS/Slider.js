...
export default class Slider extends PureComponent {
	static propTypes = {
		...
		/**
		 * The color used for the buffer value
		 */
	...
	state = {
		...
		bufferValue: new Animated.Value(this.props.bufferValue || 0),
	};
	...
	componentWillReceiveProps(nextProps) {
		...
		const newBufferValue = nextProps.bufferValue;

		if (this.props.value !== newValue) {
			if (this.props.animateTransitions) {
				this._setCurrentValueAnimated(newValue);
				this._setCurrentBufferValueAnimated(newBufferValue);
			} else {
						...
						style={[
							{
								...
								width: this.state.containerSize.width * this._getCurrentBufferValue(),
							},
							mainStyles.track,
							trackStyle,
						]}
	...
	_handlePanResponderGrant = (e: Object, gestureState: Object) => {
		this._previousLeft = this.props.trackPressable
	...
	_getCurrentBufferValue = () => this.state.bufferValue.__getValue();
	...
	_setCurrentBufferValueAnimated = (value: number) => {
		const animationType = this.props.animationType;
		const animationConfig = Object.assign(
			{},
			DEFAULT_ANIMATION_CONFIGS[animationType],
