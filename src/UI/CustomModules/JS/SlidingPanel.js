...
const SlidingPanelIOS = props => (
	...
	>
		<Animated.View {...props.panResponder} style={{ height: props.headerPanelHeight }}>
			{props.headerView()}
		</Animated.View>
		<View
			style={
		...
		this.state.panResponder = PanResponder.create({
			...
			onPanResponderMove: (event, gestureState) => {
				if (this.props.allowDragging) {
					...
					if (
						(Platform.OS === 'android'
							? sliderPosition + a < height - (this.props.headerLayoutHeight + 25)
							: sliderPosition + a < height - (this.props.headerLayoutHeight - 2)) &&
						sliderPosition + a > -2
					) {
						if (sliderPosition !== 0) {
