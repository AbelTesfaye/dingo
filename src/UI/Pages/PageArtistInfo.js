...
export class ArtistInfoPage extends React.Component {
	...
	componentDidMount = () => {
		utils.fetchFromEndpoint(`artistTopAlbums?name=${encodeURIComponent(this.props.artist.name)}`, response => {
			this.setState({
				artistTopAlbumsResponse: response.result,
			});
		});

		utils.fetchFromEndpoint(`artistTopTracks?name=${encodeURIComponent(this.props.artist.name)}`, response => {
			this.setState({
				artistTopTracksResponse: response.result,
			});
		});

		utils.fetchFromEndpoint(`artistInfo?name=${encodeURIComponent(this.props.artist.name)}`, response => {
			this.setState({
		...
		return (
			<View>
				...
				<View>
					...
					<AlbumList AppInstance={AppInstance} data={this.state.artistTopAlbumsResponse} />
					<Text
						style={{
							margin: 10,
						}}
					...
					<TrackList
						...
						onTrackPress={(item, index) =>
							AppInstance.startInPlayer(
								utils.convertToTrackPlayerFormat(this.state.artistTopTracksResponse.slice(index))
							)
						}
